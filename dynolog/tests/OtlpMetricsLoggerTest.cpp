...
TEST(OTLPLoggerTest, GPUAttributeMerging) {
  ...
  auto check_is_gpu = [](const std::string& key) {
    ...
    static const std::unordered_set<std::string> gpu_keys = {
        "graphics_engine_active_ratio", "sm_active_ratio", "sm_occupancy",
        "gpu_frequency_mhz", "fp16_active", "fp32_active", "fp64_active",
        "tensorcore_active", "hbm_mem_bw_util", "pcie_tx_bytes",
        "pcie_rx_bytes", "nvlink_tx_bytes", "nvlink_rx_bytes",
        "gpu_device_utilization", "gpu_memory_utilization",
        "gpu_power_draw", "dcgm_error", "minor_id", "device",
    };
...
TEST(OTLPLoggerTest, SerializeResource) {
  ...
  std::string payload =
      mgr.serializeMetrics({{"uptime", 1.0}}, {});
  ASSERT_FALSE(payload.empty());
...
TEST(OTLPLoggerTest, SerializeSkipAndNaN) {
  ...
  // Only gpu_power_draw should appear (device/minor_id are skip, NaN/Inf filtered)
...
// NOTE: End-to-end integration tests that required Docker + OTLP_INTEGRATION_TEST
// have been removed. They referenced deleted SDK APIs (logGauge, forceFlush,
// FLAGS_otlp_protocol, FLAGS_otlp_use_tls). Future integration tests for the
// new direct-protobuf implementation will be added in a later phase if needed.

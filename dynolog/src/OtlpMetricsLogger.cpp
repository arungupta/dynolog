...
static const std::unordered_map<std::string, ArmHwCounterMetricInfo>
    kArmHwCounterMetrics = {
        {"l1d_tlb",
         {"{event}", {{"level", "l1d"}, {"type", "access"}}}},
        {"l1d_tlb_refill",
         {"{event}", {{"level", "l1d"}, {"type", "miss"}}}},
        {"l1i_tlb",
         {"{event}", {{"level", "l1i"}, {"type", "access"}}}},
        {"l1i_tlb_refill",
         {"{event}", {{"level", "l1i"}, {"type", "miss"}}}},
        {"l2d_tlb",
         {"{event}", {{"level", "l2"}, {"type", "access"}}}},
        {"l2d_tlb_refill",
         {"{event}", {{"level", "l2"}, {"type", "miss"}}}},
        {"stall_backend_mem",
         {"{cycle}", {{"reason", "backend_mem"}}}},
        {"ll_cache_miss_rd",
         {"{event}", {{"level", "ll"}}}},
        {"br_mis_pred",
         {"{event}", {}}},
        ...
        {"l2d_cache_refill",
         {"{event}", {{"level", "l2"}, {"type", "miss"}}}},
        {"l3d_cache_refill",
         {"{event}", {{"level", "l3"}, {"type", "miss"}}}},
        {"FP_HP_SPEC",
         {"{operation}", {{"precision", "half"}}}},
        {"FP_SP_SPEC",
         {"{operation}", {{"precision", "single"}}}},
        {"FP_DP_SPEC",
         {"{operation}", {{"precision", "double"}}}},
        {"dtlb_walk",
         {"{event}", {{"type", "data"}}}},
        {"itlb_walk",
...
static const std::unordered_map<std::string, GPUMetricInfo> kGpuMetrics = {
    ...
    {"gpu_device_utilization",
     {"1", {{"hw.gpu.task", "device"}}, /* skip */ false, /* scale_factor */ 0.01}},
...
OTLPManager::OTLPManager() {
  ...
  endpoint_ =
      getEnvOr("OTEL_EXPORTER_OTLP_ENDPOINT", FLAGS_otlp_endpoint);
  if (endpoint_.empty()) {
  ...
  service_name_ =
      getEnvOr("OTEL_SERVICE_NAME", FLAGS_otlp_service_name);
  ...
  while (true) {
    std::string payload;
    {
      ...
      export_cv_.wait(lk, [this] {
        return !export_queue_.empty() || shutdown_.load();
      });
...
bool OTLPManager::httpPost(const std::string& payload) {
  ...
  curl_easy_setopt(
      curl, CURLOPT_TIMEOUT_MS, static_cast<long>(timeout_ms_));
...
void OTLPManager::buildResource() {
  // service.name
  addStringAttribute(
      resource_.add_attributes(), "service.name", service_name_);
  ...
  if (gethostname(hostname, sizeof(hostname)) == 0) {
    ...
    addStringAttribute(
        resource_.add_attributes(), "host.name", hostname);
  }
...
std::string OTLPManager::serializeMetrics(
  ...
  *rm->mutable_resource() = resource_;
  ...
  if (device_it != metrics.end()) {
    gpu_device_id =
        fmt::format("{}", static_cast<int64_t>(device_it->second));
  }
